<?php
// crear_pack_tiradas.php — Runaworld v6.9
// Genera packs exactos de tiradas autorizadas.
// El pack reserva/cobra coins al crearse.
// Las runas se aplican al inventario en confirmar_pack_tiradas.php.
...
define("PACK_SIZE_MAX", 50);
define("PACK_SIZE_DEFAULT", 1);
define("ELAPSED_CAP", 600);
define("PACK_MIN_INTERVAL_MS", 50);
...
function columnExistsPack(mysqli $conexion, string $table, string $column): bool {
...
function ensurePackTable(mysqli $conexion): void {
    $conexion->query("
        CREATE TABLE IF NOT EXISTS packs_tiradas (
            ...
            estado ENUM('pendiente','parcial','consumido') NOT NULL DEFAULT 'pendiente',
...
/**
 * Compatibilidad:
 * Si tu tabla antigua usa estado ENUM('creado','parcial','consumido'),
 * usamos 'creado'. Si usa 'pendiente', usamos 'pendiente'.
 */
function estadoInicialPack(mysqli $conexion): string {
    $res = $conexion->query("SHOW COLUMNS FROM packs_tiradas LIKE 'estado'");
    $row = $res ? $res->fetch_assoc() : null;
    $type = strtolower((string)($row["Type"] ?? ""));

    if (strpos($type, "'pendiente'") !== false) {
        return "pendiente";
    }

    if (strpos($type, "'creado'") !== false) {
        return "creado";
    }
...
function cargarInventarioPack(mysqli $conexion, int $jugador_id): array {
    $stmt = $conexion->prepare("
...
try {
    ...
    $estado_inicial = estadoInicialPack($conexion);
    ...
    // Runas normales.
    $condVarianteNormal = "";
    if (columnExistsPack($conexion, "runas", "variante")) {
        $condVarianteNormal = "AND COALESCE(variante, 'normal') = 'normal'";
    }
    ...
    // Runas corruptas.
    $condVarianteCorrupta = "";
    if (columnExistsPack($conexion, "runas", "variante")) {
        $condVarianteCorrupta = "OR COALESCE(variante, '') = 'corrupta'";
    }

    $condArchivoCorrupto = "";
    if (columnExistsPack($conexion, "runas", "archivo_html")) {
        $condArchivoCorrupto .= " OR LOWER(COALESCE(archivo_html, '')) LIKE '%corrupt%'";
    }
    if (columnExistsPack($conexion, "runas", "imagen")) {
        $condArchivoCorrupto .= " OR LOWER(COALESCE(imagen, '')) LIKE '%corrupt%'";
    }
    ...
    if (empty($runas_all)) {
        $conexion->rollback();
        responder(["ok" => false, "error" => "No hay runas normales activas"]);
    }
    ...
    for ($click = 1; $click <= $clicks_validos; $click++) {
        ...
        for ($b = 0; $b < $cantidad_bulk; $b++) {
            ...
            if ($runa_elegida) {
                $runa_visual = aplicarVarianteCorruptaSiTocaPack(
                    $runa_elegida,
                    $basicas_completas,
                    $corruptas_por_rareza
                );
                ...
                $rareza_contador = (string)($runa_visual["rareza"] ?? "");
    ...
    list($coins_ps_real, $points_por_seg_real, $coins_ps_max_real, $points_ps_max_real) =
    ...
    $mis_runas = cargarInventarioPack($conexion, $jugador_id);
    ...
    if (function_exists('debugEconomiaActivo') && debugEconomiaActivo($datos)) {
        $respuesta["debug_economia"] = [
            "endpoint" => "crear_pack_tiradas.php",
            "jugador_id" => $jugador_id,
            "coins_antes" => $coins,
            "coins_despues" => $coins_final,
            "coins_por_seg_antes" => $coins_por_seg,
            "coins_por_seg_despues" => $coins_ps_real,
            "elapsed_segundos" => $elapsed_efec,
            "coins_pasivos_sumados" => $coins_por_seg * $elapsed_efec,
    ...
    $stmt = $conexion->prepare("
        INSERT INTO packs_tiradas
            (jugador_id, pack_id, resultados_json, total_clicks, total_runas, consumidas, estado)
        VALUES (?, ?, ?, ?, ?, 0, ?)
    ");
    $stmt->bind_param(
        "issiis",
